    ...
    <>
      ...
      <div className={`lg:block hidden w-[15rem] h-[100vh] bg-[#ffff] relative border-r border-r-[#D8D8D880]`}>
        ...
        <div className='flex items-center gap-x-2 m-5'>
          <h1 className='text-xl font-semibold'>My Option Alert</h1>
        </div>

        <div className='mt-10'>
          {navData?.map((i) => (
            <Link to={`/dashboard/${i.link}`} key={i.id} className={`flex ${location == i.link && "bg-[#E6F5EA]"} px-5 p-2  items-center gap-x-3 mb-2 cursor-pointer ${location == i.link ? "text-[#049F30]" : "text-[#8F8F8F]"}`}>
              <div>{i.icon}</div>
      ...
      {/* Mobile Sidebar */}
      {
        isNavOpen && (
          <div className={`lg:hidden block w-[14rem] z-50 h-[100vh] bg-[#0F0F0F] fixed top-0 left-0 transition-all duration-300 ease-in-out ${isNavOpen ? 'transform translate-x-0' : 'transform -translate-x-full'}`} ref={sidebarRef}>
            ...
            <div className='flex items-center gap-x-2 m-5'>
              <h1 className='text-xl font-semibold text-white'>My Option Alert</h1>
            </div>
